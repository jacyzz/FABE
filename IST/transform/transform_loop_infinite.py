...
def cvt_infinite_while(node, code):
    ...
    if node.type == "for_statement":
        ...
        child_index = (
            3
            + (4 - abc.count(None))
            - (abc[0] is not None and abc[0].type == declaration_map[get_lang()])
        )
        res = [
            (
                node.children[child_index].end_byte,
                node.children[0].start_byte - node.children[child_index].end_byte,
            )
        ]  # Delete for(a; b; c)
        ...
        if abc[2] is not None and abc[3] is not None:  # If there is a c
            if (
                abc[3].type == block_map[get_lang()]
            ):  # Compound sentences are inserted in the first sentence of Yves Blake
        ...
        if len(compound_node.children) <= 1:
            return
