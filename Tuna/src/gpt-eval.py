...
def gpt_eval(
    gpt_name, prompt, temperature=0.0, top_p=1.0, max_tokens=2048, sleep_time=5
):
    while True:
        try:
    ...
    if os.path.exists(out_path):
        ...
        for i, (instruct, resp, id) in enumerate(zip(instructs, resps, ids)):
            if id not in existing_dict:
                ...
                count += 1
        logging.info(
            f"Filtered {count} prompts that have been evaluated before. {len(filtered_instruct)} prompts left."
        )
    else:
        existing_data = []
        ...
        filtered_id = ids
        logging.info(
            f"No existing evaluation file found. All prompts will be evaluated."
        )
...
class GroupEval:
    ...
    def group_eval(self):
        ...
        format_instructions, responses, rouge, ids = load_generated_data(
            self.path, num_examples=self.num_examples
        )
        format_instructions, responses, ids, existing_data = filter_already_evaluated(
            out_path, format_instructions, responses, ids
        )

        c = 0
        for i, (instruct, resp, ID) in enumerate(
            zip(format_instructions, responses, ids)
        ):
            ...
            rank_str = (
                gpt_resp.lower().split("rank:")[1].strip()
                if "rank:" in gpt_resp.lower()
                else gpt_resp.lower().split("ranking:")[1].strip()
            )
