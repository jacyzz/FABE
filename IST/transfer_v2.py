...
class IST:
    def __init__(self, language, expand=0):
        ...
        from transform_v2.config import transformation_operators as op
        from transform_v2.lang import set_lang, set_expand
...
if __name__ == "__main__":
    sys.path.append("/home/nfs/u2023-zlb/FABE/IST/transform_v2")
    from transform_deadcode import DeadCode

    with open("./test_code/test.c", "r") as f:
        code = f.read()

    ist = IST("c")
    transformer = DeadCode("c", "if-233")
    # ? -3.1 -1.1 0.5 7.2 8.1 9.1 11.3 3.4 4.4 10.7
    style = "-1.1"

    pcode, succ = transformer.tranform(code)
    print(f"succ = {succ}")
    print(pcode)
